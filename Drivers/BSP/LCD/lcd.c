...
void lcd_show_num(uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color)
{
    ...
    for (t = 0; t < len; t++)   /* 按总显示位数循环 */
    {
        ...
        if (enshow == 0 && t < (len - 1))   /* 没有使能显示,且还有位要显示 */
        {
            if (temp == 0)
            {
                lcd_show_char(x + (size / 2)*t, y, ' ', size, 1, color);/* 显示空格,占位 */
                continue;   /* 继续下个一位 */
            }
            else
            {
        ...
        lcd_show_char(x + (size / 2)*t, y, temp + '0', size, 1, color); /* 显示字符 */
    }
}
 ...
 * @param       image_data: 图片数据指针 (16 位色)
 * @retval      无
 * @note        使用 Image2Lcd 生成图片数据
 */
void lcd_show_image(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t *image_data)
{
    uint16_t i, j;
    uint16_t color;
    ...
    for (i = 0; i < height; i++)
    {
        ...
        {
            /* 读取 16 位颜色值（小端模式） */
            color = ((uint16_t)image_data[(i * width + j) * 2 + 1] << 8) | 
                     (uint16_t)image_data[(i * width + j) * 2];
            LCD->LCD_RAM = color;
        }
    }
}
...
/**
 * @brief       查找汉字字模 (新二维数组版本)
 * @param       str: 要查找的汉字 (2 字节 GB2312 编码)
 * @param       size: 字体大小
 * @retval      字模在 chinese 数组中的索引，未找到返回 0xFF
 */
static uint8_t find_chinese_font_index(const char *str, uint8_t size)
{
    uint16_t i;
    uint8_t hanzi_code[2];
    
    hanzi_code[0] = (uint8_t)str[0];
    hanzi_code[1] = (uint8_t)str[1];
    
    /* 遍历索引表查找 */
    for (i = 0; i < ChineseIndexCount; i++)
    {
        if (ChineseFontIndex[i][0] == hanzi_code[0] && 
            ChineseFontIndex[i][1] == hanzi_code[1])
        {
            return ChineseFontIndex[i][2];  /* 返回数组索引 */
        }
    }
 ...
 * @retval      无
 */
static void lcd_show_single_chinese(uint16_t x, uint16_t y, uint8_t size, const char *hanzi, uint16_t color, uint16_t backgroundColor)
{
    ...
    /* 查找字模索引 */
    font_index = find_chinese_font_index(hanzi, size);
    if (font_index == 0xFF) 
    {
        /* 未找到字模，显示方框 */
        lcd_draw_rectangle(x, y, x + size - 1, y + size - 1, color);
        /* 在方框内画叉 */
        lcd_draw_line(x, y, x + size - 1, y + size - 1, color);
        lcd_draw_line(x + size - 1, y, x, y + size - 1, color);
        return;
    }
    
    /* 获取字模数据指针 */
    font_data = chinese[font_index];
    
    /* 计算每行占用的字节数 */
    uint8_t bytes_per_line = size / 8;
    
    /* 设置窗口 */
    lcd_set_window(x, y, size, size);
    lcd_write_ram_prepare();
    
    /* 显示汉字 - 逐行处理（适配 PCtoLCD2002 逐行式字模）
     * PCtoLCD2002 逐行式格式：
     * - 第 1-2 字节：第 1 行（从左到右 8 个像素）
     * - 第 3-4 字节：第 2 行
     * - ...共 16 行，32 字节
     * - 每个字节：bit0=左，bit7=右（逆向，低位在前）
     */
    for (i = 0; i < size; i++)  /* 遍历每一行 */
    {
        for (j = 0; j < bytes_per_line; j++)  /* 遍历该行的每个字节 */
        {
            temp = font_data[i * bytes_per_line + j];
 ...
 * @param       color: 颜色
 * @retval      无
 * @note        需要配合字库使用，使用 PCtoLCD2002 生成字模
 */
void lcd_show_chinese_string(uint16_t x, uint16_t y, uint8_t size, char *str, uint16_t color, uint16_t backgroundColor)
{
    uint16_t current_x = x;
    uint16_t current_y = y;
    uint16_t screen_width = lcddev.width;
    
    while (*str)
    {
        /* 判断是否为汉字 (GB2312 编码，首字节>0x80) */
        if ((unsigned char)*str > 0x80 && *(str + 1))
        {
            /* 是汉字 */
            char hanzi[3] = {0};
            hanzi[0] = *str;
            ...
            /* 检查是否需要换行 */
            if (current_x + size > screen_width)
            {
                current_x = x;
                current_y += size;
            }
            ...
            /* 显示单个汉字 */
            lcd_show_single_chinese(current_x, current_y, size, hanzi, color, backgroundColor);
            current_x += size;
            str += 2;  /* 跳过 2 个字节 */
        }
        else
        {
            /* 是 ASCII 字符，使用原有函数显示 */
            lcd_show_char(current_x, current_y, *str, size, 1, color);
            current_x += size / 2;
            str++;
        }
    }
 ...
 * @param       color: 颜色
 * @retval      无
 * @note        与 lcd_show_chinese_string 功能相同，为了兼容性保留
 */
void lcd_show_chinese(uint16_t x, uint16_t y, uint8_t size, const char *str, uint16_t color, uint16_t backgroundColor)
{
    lcd_show_chinese_string(x, y, size, (char*)str, color, backgroundColor);
}
