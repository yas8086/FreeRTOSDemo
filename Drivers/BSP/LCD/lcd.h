...
/* 新增 API 函数 */
void lcd_show_image(uint16_t x, uint16_t y, uint16_t width, uint16_t height, const uint8_t *image_data);  /* 显示图片 */
void lcd_show_chinese(uint16_t x, uint16_t y, uint8_t size, const char *str, uint16_t color, uint16_t backgroundColor);              /* 显示汉字 */
void lcd_show_chinese_string(uint16_t x, uint16_t y, uint8_t size, char *str, uint16_t color, uint16_t backgroundColor);             /* 显示汉字字符串 */
#endif
